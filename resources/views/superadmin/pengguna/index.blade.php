        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
            <div>
                ...
                <p class="mt-2 text-lg text-slate-500">
                    Kelola akun pengguna terdaftar dalam sistem SIPANDA.
                </p>
            </div>
        ...
        <div class="bg-white rounded-[28px] border border-slate-100 shadow-sm p-6">
            <form method="GET" action="{{ route('superadmin.pengguna.index') }}"
                ...
                <div class="lg:col-span-1">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Bidan..."
                ...
                <div class="lg:col-span-1">
                    ...
                    <select name="id_posyandu"
                        ...
                        @foreach($posyanduList as $item)
                            <option value="{{ $item->id_posyandu }}" {{ request('id_posyandu') == $item->id_posyandu ? 'selected' : '' }}>
                                {{ $item->nama_posyandu }}
                            </option>
                        @endforeach
                    </select>
                                ...
                                <tr class="hover:bg-slate-50 transition">
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="flex items-center justify-end gap-3 pr-6">
                                            ...
                                            <form method="POST" action="{{ route('superadmin.pengguna.destroy', $item->id_user) }}"
                                                onsubmit="return confirm('Hapus akun bidan ini?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
