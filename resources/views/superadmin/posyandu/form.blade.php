                ...
                @push('scripts')
                    <script>
                        document.addEventListener('DOMContentLoaded', function () {
                            // 1. Data Wilayah (Pastikan tidak ada tanda kutip yang tertinggal/unexpected string)
                            const dataWilayah = {
                            ...
                            // Ambil data lama dari database (untuk mode edit)
                            const oldKecamatan = "{{ $posyandu->kecamatan ?? '' }}";
                            const oldDesa = "{{ $posyandu->desa_kelurahan ?? '' }}";

                            // 2. Fungsi mengisi Kecamatan
                            function populateKecamatan() {
                                ...
                                Object.keys(dataWilayah).sort().forEach(kec => {
                                    ...
                                    // Set terpilih jika sedang mode edit
                                    if (kec === oldKecamatan) option.selected = true;
                                    kecamatanSelect.appendChild(option);
                                });

                                // Jika ada data lama, langsung isi dropdown desa
                                if (oldKecamatan) {
                                    updateDesa(oldKecamatan, oldDesa);
                                }
                            }

                            // 3. Fungsi mengisi Desa
